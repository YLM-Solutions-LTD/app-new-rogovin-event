(function () {
  ...
  function renderForm(ctx, options = {}) {
    ...
    const locationsMarkup = locations.length ? locations.map((location, index) => `<button class="location${index === 0 ? " selected" : ""}" type="button" data-id="${esc(location.id ?? "")}" aria-pressed="${index === 0}"${preview ? " disabled" : ""}><span class="location-pin" aria-hidden="true">⌖</span><span>${esc(location.name)}</span>${location.mine ? '<small>המיקום שלי</small>' : ""}</button>`).join("") : '<p class="empty-options">המיקום שלי יוצג לאחר פתיחת היישום מתוך SimplyLog.</p>';
    const categoriesMarkup = categories.length ? categories.map((c) => `<button class="category" type="button" data-id="${esc(c.id)}" aria-pressed="false"${preview ? " disabled" : ""}><span class="check">✓</span><span class="icon" aria-hidden="true">${esc(c.icon)}</span><span>${esc(c.name)}</span></button>`).join("") : '<p class="empty-options">הקטגוריות יוצגו כאן מתוך SimplyLog.</p>';
    appEl.innerHTML = `<div class="shell"><section class="panel">${previewNotice}<ol class="progress" aria-label="תהליך הדיווח"><li class="active"><span>1</span></li><li><span>2</span></li><li><span>3</span></li><li><span>4</span></li></ol><header><h1>דיווח על אירוע חדש</h1><p>בחרו את סוג האירוע, הוסיפו פרטים ושלחו.</p></header><form id="event-form" novalidate><fieldset class="field plain-fieldset"><legend class="legend">מיקום האירוע</legend><div class="locations">${locationsMarkup}</div></fieldset><fieldset class="field plain-fieldset"><legend class="legend">סוג האירוע <span class="required">*</span></legend><div class="categories">${categoriesMarkup}</div><input id="category" type="hidden"></fieldset><div class="field"><label for="description">תיאור האירוע <span class="required">*</span></label><textarea id="description" maxlength="1000" placeholder="פרטו מה קרה, היכן ומתי"${preview ? " disabled" : ""}></textarea></div><div class="field"><label class="dropzone">⌁ <strong>צרפו קבצים או תמונות</strong><br><span class="hint">עד 10MB לקובץ</span><input id="attachments" type="file" multiple accept="image/*,.pdf"${preview ? " disabled" : ""}></label><ul id="file-list" class="files"></ul></div><p id="validation" class="validation" role="alert"></p><button class="submit" type="submit"${preview ? " disabled" : ""}>שליחת הדיווח</button></form></section></div>`;
    if (preview) return;
    let selected = "";
